...
<div style="max-width:960px;margin:0 auto">
  ...
  // ── Bug Report (JIRA / VersionOne) ───────────────────────────────────────

  const generateBugReport = () => {
    if (!summary) return;
    const now = new Date().toLocaleString();
    const pdfName = slots.pdf?.name ?? 'Unknown';
    const failResults = results.filter(r => !acceptedIds.has(r.id) && r.status !== 'PASS');
    const failCount = failResults.filter(r => r.status === 'FAIL').length;
    const naCount = failResults.filter(r => r.status === 'NA').length;
    const priority = failCount >= 5 ? 'High' : failCount >= 2 ? 'Medium' : 'Low';

    const sep = (char = '-', len = 60) => char.repeat(len);

    // Group open failures by category
    const grouped = failResults.reduce((acc: Record<string, ValidationResult[]>, r) => {
      if (!acc[r.category]) acc[r.category] = [];
      acc[r.category].push(r);
      return acc;
    }, {} as Record<string, ValidationResult[]>);

    const findingsBlock = (Object.entries(grouped) as [string, ValidationResult[]][]).map(([cat, items]) => {
      const rows = items.map(r =>
        `  [${r.id}] ${r.field}\n` +
        `  Status  : ${r.status}\n` +
        (r.page != null ? `  Page    : p${r.page}\n` : '') +
    ...
    const docBlock = docAnalysis ? [
      `Overall Quality : ${docAnalysis.overallQuality}`,
      `Tone            : ${docAnalysis.tone}`,
      `                  ${docAnalysis.toneNotes}`,
      `Completeness    : ${docAnalysis.completeness}%`,
      `                  ${docAnalysis.completenessNotes}`,
      ...(docAnalysis.missingBlocks.length > 0
        ? ['Missing Blocks  :', ...docAnalysis.missingBlocks.map(b => `  - ${b}`)]
        : []),
      ...(docAnalysis.suggestions.length > 0
        ? ['Suggestions     :', ...docAnalysis.suggestions.map(s => `  - ${s}`)]
        : []),
    ].join('\n') : '(Not run — enable Document Analysis and re-validate)';

    const lines = [
      '================================================================',
      'OUTPUT VALIDATION — BUG REPORT',
      `Generated : ${now}`,
      '================================================================',
      '',
      sep(),
      ...
      sep(),
      `Title       : Output Validation Failed — ${pdfName} (${failCount} failure${failCount !== 1 ? 's' : ''})`,
      `Type        : Bug`,
      `Priority    : ${priority}`,
      `Labels      : output-validator, ccm-qa, document-generation`,
      `Environment : [replace with: QA / UAT / Production]`,
      '',
      sep(),
      'VALIDATION SUMMARY',
      sep(),
      `PDF         : ${pdfName}`,
      `Input Data  : ${slots.data?.name ?? '—'}`,
      `Test Suite  : ${slots.testcases?.name ?? '—'}`,
      ...(slots.rules?.name ? [`Business Rules: ${slots.rules.name}`] : []),
      `Mode        : ${mode === 'ai' ? 'AI-Assisted' : 'Deterministic'}`,
      `Validated   : ${now}`,
      '',
      `Total       : ${summary.total}`,
      ...
      `N/A         : ${summary.na}`,
      ...(acceptedIds.size > 0 ? [`Accepted    : ${acceptedIds.size} (excluded from pass rate)`] : []),
      `Pass Rate   : ${passRate}%`,
      ...(summary.skipped > 0 ? [`Skipped     : ${summary.skipped} (field not in input data)`] : []),
      '',
      sep(),
      `OPEN FAILURES (${failResults.length} total — FAIL: ${failCount}, N/A: ${naCount})`,
      sep(),
      failResults.length === 0 ? '(No open failures — all issues have been accepted or resolved)' : findingsBlock,
      '',
      sep(),
      'DOCUMENT ANALYSIS',
      sep(),
      ...
      '',
      sep(),
      'ATTACHMENTS (add to ticket)',
      sep(),
      `  - ${pdfName}  (PDF being validated)`,
      ...(slots.data?.name ? [`  - ${slots.data.name}  (input data)`] : []),
      ...(slots.testcases?.name ? [`  - ${slots.testcases.name}  (test suite)`] : []),
      `  - validation-report-*.html  (full report — generate via Output Validator)`,
      '',
      '================================================================',
      'Generated by: Document Intelligence Hub — Output Validator',
      '================================================================',
    ];
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `bug-report-${pdfName.replace(/\.pdf$/i, '')}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };
          ...
          <div className="flex-[3] min-w-0 flex flex-col gap-3 min-h-0">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex-shrink-0">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button onClick={generateBugReport} disabled={results.length === 0}
                  title="Download a pre-filled bug report for JIRA or VersionOne"
                  className="flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg border border-amber-200 dark:border-amber-800/60 hover:bg-amber-50 dark:hover:bg-amber-900/20 disabled:opacity-40 text-amber-700 dark:text-amber-400 text-xs font-medium transition-colors"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 12.75c1.148 0 2.278.08 3.383.237 1.037.146 1.866.966 1.866 2.013 0 3.728-2.35 6.75-3.749 6.75-1.4 0-3.75-3.022-3.75-6.75 0-1.047.83-1.867 1.866-2.013A24.204 24.204 0 0112 12.75zm0 0c2.883 0 5.647.508 8.207 1.44a23.91 23.91 0 01-1.152 6.06M12 12.75c-2.883 0-5.647.508-8.208 1.44a23.91 23.91 0 001.153 6.06M12 12.75a2.25 2.25 0 002.248-2.354M12 12.75a2.25 2.25 0 01-2.248-2.354M12 8.25c.995 0 1.971-.08 2.922-.236.403-.066.74-.358.795-.762a3.778 3.778 0 00-.399-2.25M12 8.25c-.995 0-1.97-.08-2.922-.236-.402-.066-.74-.358-.795-.762a3.778 3.778 0 01.4-2.25m0 0a3.75 3.75 0 016.343 0M12 5.764a3.75 3.75 0 00-6.343 0" />
                  </svg>
                  Bug Report (JIRA/V1)
                </button>
              </div>
