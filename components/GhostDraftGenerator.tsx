...
const GhostDraftGenerator: React.FC = () => {
  ...
  const allFilesReady = docxFile && csvFile && xsdFile && gdRefFile;

  const handleGenerate = async () => {
    if (!docxFile || !csvFile || !xsdFile) return;
  ...
  const baseName = docxFile?.name.replace(/\.docx$/i, '') ?? 'document';
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">All four files are required · CSV, XSD and .gd stay loaded across multiple documents</p>
          </div>
        </div>

        {/* 2×2 grid — all required */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          <UploadZone
            label="Word Document"
            description="Drop .docx file here"
            accept=".docx"
            file={docxFile}
            onFile={setDocxFile}
            color="blue"
          ...
          />
          <UploadZone
            label="XPath Mapping CSV"
            description="From XPath Extractor output"
            accept=".csv"
            file={csvFile}
            onFile={setCsvFile}
            color="emerald"
          ...
          <UploadZone
            ...
            description="Reference .gd file for correct Model Library GUIDs"
            accept=".gd,.xml"
            file={gdRefFile}
            onFile={setGdRefFile}
            color="indigo"
          ...
          {result && (
            <>
              <button
                ...
                title="Clear DOCX and result — CSV, XSD and .gd stay loaded"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                </svg>
